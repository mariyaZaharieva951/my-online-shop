...
const Benefits = () => {
  return (
    <div className="box mx-auto flex lg:flex-row flex-col gap-[85px] py-[50px]">
      {benefits.map((benefit, index) => (
        <div key={index} className="flex items-center h-12 gap-5">
          ...
          <div className="ml-2">
            <h4 className="lg:text-lg font-bold">{benefit.title}</h4>
            <p className="lg:text-base text-gray">{benefit.text}</p>
          </div>
        </div>
      ))}
    </div>
